...
🎨 Генерируйте изображения из текстовых запросов в режиме <b>👩‍🎨 Художник</b> /mode
...
async def start_handle(update: Update, context: CallbackContext):
    ...
    reply_text = "Привет! Я бот <b>ChatGPT</b>, реализованный с OpenAI API 🤖\n\n"
...
async def _vision_message_handle_fn(
    ...
    if use_new_dialog_timeout:
        if (datetime.now() - db.get_user_attribute(user_id, "last_interaction")).seconds > config.new_dialog_timeout and len(db.get_dialog_messages(user_id)) > 0:
            db.start_new_dialog(user_id)
            await update.message.reply_text(f"Начинаю новый диалог из-за таймаута (режим <b>{config.chat_modes[chat_mode]['name']}</b>) ✅", parse_mode=ParseMode.HTML)
    db.set_user_attribute(user_id, "last_interaction", datetime.now())
...
async def unsupport_message_handle(update: Update, context: CallbackContext, message=None):
    error_text = f"Я не умею читать файлы или видео. Отправьте картинку в обычном режиме (Быстрый режим)."
    logger.error(error_text)
    await update.message.reply_text(error_text)
    return
